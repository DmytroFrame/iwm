...
fn skip_lisen(stream: &TcpStream) {
    let size = read_int(stream);
    ...
    match id {
        0 => {
            ...
            if status == 1 {
                ...
                write_bytes(stream, create_level(0, 0));
                ...
                // write_bytes(stream, b"".to_vec());
                // let mut buf = Vec::new();
                // let message = r###"{ "text": "Сервер буде відкрити 5 вересня, за цей час ти можеш подрочити, я буду дивитись на твою вебку, бо я вірус)) шясливо..", "bold": true }"###;
                // buf.push(0);
                ...
                write_bytes(stream, create_game_event(3, 3)); // set survaivel mode
...
fn status_response(stream: &TcpStream, protocol: i32) {
    ...
    string.push_str(r###" }, "players": { "max": -7, "online": 1000, "sample": [ { "name": "DmytroFrame", "id": "4566e69f-c907-48ee-8d71-d7ba5aa00d20" } ] }, "description": { "text": "KEPSKIY SERVER" }, "favicon": "data:image/png;base64,<>", "previewsChat": false }"###);
